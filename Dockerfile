...
# Upgrade CANN toolkit and ops package to 9.2.0-beta.2
ARG TARGETPLATFORM
RUN ARCH=$(case "${TARGETPLATFORM}" in \
    ...
    CANN_TOOLKIT_URL=https://ascend-repo.obs.cn-east-2.myhuaweicloud.com/CANN/CANN%209.2.T3/Ascend-cann-toolkit_9.2.0-beta.2_linux-${ARCH}.run && \
    wget --quiet --header="Referer: https://www.hiascend.com/" ${CANN_TOOLKIT_URL} -O ~/Ascend-cann-toolkit-9.2.0.run && \
    chmod +x ~/Ascend-cann-toolkit-9.2.0.run && \
    ~/Ascend-cann-toolkit-9.2.0.run --quiet --install --install-for-all && \
    rm -f ~/Ascend-cann-toolkit-9.2.0.run && \
