...
class KVCacheRecvingThread(threading.Thread):
    ...
    def _get_group_kv_caches(self, group_idx: int, layer_indices: list[int] | None = None) -> dict[str, Any]:
        ...
        selected_layer_indices = set(group_layer_indices if layer_indices is None else layer_indices)
        # The separately named SFA indexer cache shares MLA's real layer index,
        # but it must not be included in MLA cache reformatting.
        return {
    ...
    def _get_transfer_block_ids(self, block_ids: BlockIds, prompt_len: int) -> BlockIds:
        ...
        for blocks, group_info in zip(block_ids, self.group_transfer_info):
            if group_info.is_state_group:
                ...
                # Each scheduler-visible block is shared by all CP ranks and
                # therefore covers cp_size times one rank's token span.
...
class MooncakeConnectorWorker:
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        self.kv_caches = kv_caches
        # Keep the separately exposed SFA indexer group while registering so
        # its cache tensors are appended to the same real model-layer entry as
        # MLA. The extra transfer group is removed after registration below.
        ...
        if self.enable_sfa_dcp_replicated_indexer:
            self.kv_group2layeridx = {
                ...
                for group_idx, group in enumerate(
                    group
                    for group in self.kv_group2layeridx.values()
                    if group[0]["kv_cache_spec_type"] != "AscendSFAIndexerCacheSpec"
                )
            }
