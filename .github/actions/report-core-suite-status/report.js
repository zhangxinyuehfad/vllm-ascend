module.exports = async function report({ github, core, context, branch }) {
  ...
  // Read core suite test names from config.
  ...
  // List all jobs in the current workflow run.
  let allJobs = [];
  let page = 1;
  while (true) {
  ...
  // Check "Application startup complete" in job logs.
  const checkStartup = async (job) => {
    try {
      const resp = await github.rest.actions.downloadJobLogsForWorkflowRun({
        owner, repo, job_id: job.id,
      });
      const text = Buffer.isBuffer(resp.data) ? resp.data.toString('utf8') : String(resp.data);
  ...
  // Check if test session actually ran in job logs.
  const checkTestsRun = async (job) => {
    try {
      const resp = await github.rest.actions.downloadJobLogsForWorkflowRun({
        owner, repo, job_id: job.id,
      });
      const text = Buffer.isBuffer(resp.data) ? resp.data.toString('utf8') : String(resp.data);
      return text.includes('test session starts');
    } catch (e) {
      core.warning(`Failed to download logs for job "${job.name}": ${e.message}`);
      return false;
    }
  ...
  for (const test of coreTests) {
    ...
    for (const job of matched) {
      ...
      if (conclusion === 'success') {
        ...
        const hasStartup = await checkStartup(job);
        if (hasStartup) {
          core.info('  -> Has "Application startup complete" -> non-core issue, pass');
          results.push({ test, status: 'pass' });
        } else {
          const hasTestsRun = await checkTestsRun(job);
          if (hasTestsRun) {
