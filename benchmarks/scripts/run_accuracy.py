...
def generate_md(model_name, tasks_list, args, datasets):
    ...
    preamble = f"""# 🎯 {model}
  <div>
    <strong>vLLM version:</strong> vLLM: {args.vllm_version} ([{args.vllm_commit}]({args.vllm_commit_url})), 
    vLLM Ascend: {args.vllm_ascend_version} ([{args.vllm_ascend_commit}]({args.vllm_ascend_commit_url}))<br>
  <br>
  </div>
  <div>
      <strong>vLLM Engine:</strong> V{args.vllm_use_v1} <br>
  </div>
  <div>
      <strong>Software Environment:</strong> CANN: {args.cann_version}, 
      PyTorch: {args.torch_version}, 
      torch-npu: {args.torch_npu_version} <br>
  </div>
  <div>
